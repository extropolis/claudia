...
export class DeploymentCatalog {
     ...
     */
    async findDeploymentFor(modelName) {
    ...
    createModelFromDeployment(deployment) {
    ...
    parseCreatedAt(createdAtString) {
    ...
    async fetchAndCacheDeployments() {
    ...
    filterRunningDeployments(resources) {
        return resources.filter(d => d.status === DEPLOYMENT_STATUS_RUNNING &&
            this.hasValidModelName(d));
    }
    hasValidModelName(deployment) {
        return !!deployment?.details?.resources?.backendDetails?.model?.name;
    }
    extractModelName(deployment) {
        return deployment.details.resources.backendDetails.model.name;
    }
}
