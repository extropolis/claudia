...
export class RequestTransformer {
    transform(input) {
        ...
        const { model: _model, stream: _stream, reasoning_effort, context_management: _contextManagement, ...rest } = input;
     ...
     */
    sanitizeCacheControl(data) {
        let result = { ...data };
        // Sanitize tools
        if (data.tools && Array.isArray(data.tools)) {
            let strippedCount = 0;
            const sanitizedTools = data.tools.map((tool) => {
        ...
        if (data.system && Array.isArray(data.system)) {
            let strippedCount = 0;
            const sanitizedSystem = data.system.map((block) => {
    ...
    convertReasoningEffort(effort, maxTokens) {
        const budgetMap = {
    ...
    clampThinking(data) {
        ...
        if (!thinking || thinking.budget_tokens == null)
            return data;
