...
export default class HaiProxyPlugin {
    ...
    async initialize(context) {
        ...
        // Dynamically import hyperspace proxy with cache busting for hot reload
        const proxyPath = join(__dirname, 'hyperspace-proxy', 'index.js');
        const cacheBust = `?t=${Date.now()}`;
        const proxyModule = await import(proxyPath + cacheBust);
        this.proxyRouter = proxyModule.createHyperspaceProxy(context.configStore);
        ...
        if (storedConfig?.apiKey) {
            ...
            if (running) {
                ...
                return; // Already running, nothing more to do
            }
        }

        // Auto-start proxy if API mode is set to hyperspace-proxy
        const config = context.configStore.getConfig();
        if (config.apiMode === 'hyperspace-proxy') {
            context.logger.info('API mode is hyperspace-proxy, auto-starting HAI proxy...');
            try {
                const result = await this.startProxy();
                if (result.success) {
                    context.logger.info('HAI proxy auto-started successfully', { apiKey: result.apiKey });
                } else {
                    context.logger.error('Failed to auto-start HAI proxy', { error: result.error });
                }
            } catch (error) {
                context.logger.error('Exception during HAI proxy auto-start', { error: error.message });
            }
        }
    }
    ...
    getRouter() {
        ...
        // Test connection endpoint (used by frontend Settings)
        router.post('/test', async (req, res) => {
            try {
                ...
                const result = await this.testConnection({ proxyUrl, apiKey });
                if (result.success) {
                    res.json(result);
                } else {
                ...
                const message = error instanceof Error ? error.message : String(error);
                this.context.logger.error('Failed to test HAI proxy connection', { error: message });
                res.status(500).json({ success: false, error: message });
            }
        });

        // Fetch models endpoint (used by frontend Settings)
        router.post('/models', async (req, res) => {
            try {
                ...
                const message = error instanceof Error ? error.message : String(error);
                this.context.logger.error('Failed to fetch HAI proxy models', { error: message });
                res.status(500).json({ success: false, error: message });
            }
        });
    ...
    async startProxy() {
        ...
        const hyperspaceConfig = {
            proxyUrl: HAI_PROXY_URL,
            apiKey,
            model: this.context.configStore.getHyperspaceProxy()?.model || 'anthropic--claude-4.5-sonnet',
