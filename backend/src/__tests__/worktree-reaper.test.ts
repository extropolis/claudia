...
// Shells out to real git repeatedly; the 10s project default flakes under load
// and on slow CI runners. File-scoped — the project default is unchanged.
...
describe('removeWorktreeWithUnlockRetry (real git)', () => {
    ...
    it('removes a clean worktree on the first attempt', async () => {
        ...
        expect(git(['worktree', 'list', '--porcelain'], repo)).not.toContain(resolve(wt));
    });
    ...
    it('CHARACTERIZATION (known data-loss surface): the retention sweep --force-removes worktrees holding uncommitted work', async () => {
        ...
        // Documented, not endorsed — see the report.
        const repo = makeRepo();
