...
import { validateConfigUpdate, validateWorkspacePath, sanitizePrompt } from '../validation.js';
...
describe('validateConfigUpdate', () => {
  ...
  it('should validate mcpServers array', () => {
    ...
    expect(validateConfigUpdate({ mcpServers: 'not-array' }).error).toBe(
  ...
  it('should validate backend enum', () => {
    ...
    expect(validateConfigUpdate({ backend: 'invalid-backend' }).error).toContain(
  ...
  it('should validate MCP server type enum', () => {
    ...
    expect(
      validateConfigUpdate({
        mcpServers: [
          { name: 'test', type: 'streamableHttp', url: 'http://localhost:3000', enabled: true },
        ],
      }).valid,
    ).toBe(true);
  ...
  it('should validate MCP server optional fields', () => {
    ...
    expect(
      validateConfigUpdate({
        mcpServers: [
          { name: 'test', command: 'cmd', enabled: true, autoApprove: ['tool1', 'tool2'] },
        ],
      }).valid,
    ).toBe(true);
