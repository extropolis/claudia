...
import { createTestEnv, waitFor, type TestEnv, type WSClient } from './helpers/ws-harness.js';
...
describe('task:select / task:input / task:resize', () => {
    let taskId: string;
...
describe('task:stop / task:stopAll / task:interrupt', () => {
...
describe('task:rename + task:reorder persist across a server restart', () => {
...
describe('task:revert guardrails at the WS layer', () => {
...
describe('task:deleteRequest / task:deleteRejected broadcast to the waiting MCP client', () => {
...
describe('archive → list → restore → continue → delete round trip', () => {
...
describe('task:disconnect / task:reconnect', () => {
