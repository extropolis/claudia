...
describe('GET/PUT /api/config', () => {
    ...
    it('GET returns the defaulted config shape', async () => {
        ...
        // BUG: ConfigStore.normalize() never copies `worktreeRetentionDays`, so
        // DEFAULT_CONFIG's 30 is dead code and the API serves `undefined` until
        // the field is explicitly PUT. The sweep itself still defaults to 30 via
        // getWorktreeRetentionDays()'s own fallback, so only the API/UI contract
        // is wrong. Asserted as-is; flip to `toBe(30)` when normalize() is fixed.
        expect(body.worktreeRetentionDays).toBeUndefined();
    ...
    it('PUT rejects an out-of-range worktreeRetentionDays with 400 and leaves the value alone', async () => {
        const tooBig = await h.send<{ error: string }>('PUT', '/api/config', { worktreeRetentionDays: 4000 });
        expect(tooBig.status).toBe(400);
        expect(tooBig.body.error).toMatch(/worktreeRetentionDays must be an integer between 0 and 3650/);

        const fractional = await h.send<{ error: string }>('PUT', '/api/config', { worktreeRetentionDays: 1.5 });
        expect(fractional.status).toBe(400);

        const negative = await h.send<{ error: string }>('PUT', '/api/config', { worktreeRetentionDays: -1 });
        expect(negative.status).toBe(400);

        // Nothing was persisted by the rejected calls (see the normalize() bug
        // noted above for why the unset value reads back as undefined).
        const after = await h.req<AppConfigBody>('/api/config');
        expect(after.body.worktreeRetentionDays).toBeUndefined();
    });
    ...
    it('PUT round-trips a valid update', async () => {
        ...
        expect(put.body.worktreeRetentionDays).toBe(7);
        ...
        expect(get.body.worktreeRetentionDays).toBe(7);
...
// ===========================================================================
describe('live task /debug and /output', () => {
    let live: Harness;
    let liveWs: string;
    let taskId: string;
