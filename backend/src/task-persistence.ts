...
export class TaskPersistenceManager {
  ...
  loadPersistedTasks(): {
    ...
    try {
      ...
      for (const persisted of persistence.tasks) {
        ...
        if (persisted.outputHistory && typeof persisted.outputHistory === 'string') {
          try {
            ...
            console.error(
              `[TaskPersistence] Failed to migrate history for task ${persisted.id}:`,
              e,
            );
          }
        }
      ...
      if (persistence.archivedTasks) {
        console.log(
          `[TaskPersistence] Loading ${persistence.archivedTasks.length} archived tasks (metadata only)`,
        );
        ...
        if (result.migratedCount > 0) {
          console.log(
            `[TaskPersistence] Migrated ${result.migratedCount} archived tasks to lazy loading format`,
          );
        }
      }
    } catch (error) {
   ...
   * Save tasks to disk
   */
  saveTasks(tasks: PersistedTask[], archivedTasks: ArchivedTaskMetadata[]): void {
    try {
      ...
      console.log(
        `[TaskPersistence] Saved ${tasks.length} tasks, ${archivedTasks.length} archived (metadata only)`,
      );
    } catch (error) {
  ...
  loadTaskHistory(taskId: string, maxSize?: number): { buffer: Buffer | null; truncated: boolean } {
    ...
    try {
      ...
      const decoded = Buffer.from(base64Content, 'base64');
      console.log(
        `[TaskPersistence] Loaded tail of history for ${taskId}: ${fileSize} bytes (file) -> ${maxSize} bytes (loaded)`,
      );
