...
function resolveClaudeSpawn(): { command: string; prefixArgs: string[] } {
  ...
  console.warn(
    '[TaskSpawner] APPDATA-based Claude CLI not found, falling back to cmd.exe /c claude.cmd',
  );
...
export class TaskSpawner extends EventEmitter {
  ...
  private pendingSessionCapture: Map<
    string,
    { taskId: string; workspaceId: string; startTime: number }
  > = new Map();
  ...
  constructor(persistencePath?: string, autoReconnect: boolean = true, configStore?: ConfigStore) {
    ...
    this.historyFileMaxBytes =
      !isNaN(envMaxBytes) && envMaxBytes >= 0 ? envMaxBytes : 10 * 1024 * 1024;
    const envKeepBytes = parseInt(process.env.HISTORY_FILE_KEEP_BYTES || '', 10);
    this.historyFileKeepBytes =
      !isNaN(envKeepBytes) && envKeepBytes > 0 ? envKeepBytes : 5 * 1024 * 1024;
  ...
  private initializeBackend(): void {
    ...
    this.backend.on('task:stateChanged', (backendTask: BackendTask) => {
      ...
      if (task) {
        ...
        if (oldState !== task.state) {
          logger.debug('Task state changed via backend', {
            taskId: task.id,
            oldState,
            newState: task.state,
          });
    ...
    this.backend.on(
      'task:waitingInput',
      (taskId: string, inputType: WaitingInputType, context: string) => {
  ...
  private buildMcpConfig(
      ...
      if (serverType === 'streamableHttp' || serverType === 'http') {
        ...
        if (resolvedCommand === 'npx' && resolvedArgs.length > 0) {
          ...
          if (resolved) {
            ...
            logger.info(`Optimized MCP server "${server.name}": npx → direct node`, {
    ...
    if (claudiaMcpEnabled) {
      ...
      logger.info('Injected Claudia MCP server into MCP config', {
        path: mcpServerPath,
  ...
  syncWorkspaceMcpConfigs(workspaceIds: string[]): void {
    ...
    for (const workspaceId of workspaceIds) {
      ...
      if (resolve(workspaceId) === selfRoot) {
        logger.info("Skipping MCP sync for Claudia's own workspace (prevents tsx watch restart)", {
          workspaceId,
        });
        continue;
      }
    ...
    logger.info(`Synced MCP config to ${workspaceIds.length} workspace(s)`, {
      servers: serverNames,
    });
  }
  ...
  readTaskHistoryRange(
    ...
    try {
      ...
      return {
        data: cleanBuf.toString('utf8'),
        startOffset: startOffset + skipBytes,
        totalSize,
        isBase64Legacy: false,
      };
    } finally {
      try {
        closeSync(fd);
    ...
    try {
      for (const entry of readdirSync(historyDir)) {
        ...
        try {
          ...
          logger.warn('Failed to delete orphan history file', {
            file: entry,
    ...
    try {
      ...
      try {
        ...
        const marker = Buffer.from(
          `\x1b[90m── history trimmed (${Math.round((stat.size - start) / 1024)} KB shown of ${Math.round(stat.size / 1024)} KB) ──\x1b[0m\r\n`,
        );
      ...
      logger.warn('Failed to rotate history file', {
        file: historyPath,
        error: (e as Error).message,
      });
    }
  ...
  private async reconnectAfterSleep(): Promise<void> {
    ...
    for (let i = 0; i < allIds.length; i++) {
      try {
        ...
        logger.info('Reconnected task after sleep', {
          taskId: allIds[i],
          index: i + 1,
          total: allIds.length,
        });
      } catch (e) {
        logger.warn('Failed to reconnect task after sleep', {
          taskId: allIds[i],
          error: (e as Error).message,
        });
      }
        ...
        if (task.state === 'busy') {
          ...
          if (inputType) {
            ...
            this.captureTokenUsage(task.id)
              .catch((err) => {
                logger.error('Unhandled error in captureTokenUsage', {
                  taskId: task.id,
            ...
            this.captureGitStateAfterTask(task.id).catch((err) => {
              logger.error('Unhandled error in captureGitStateAfterTask', {
                taskId: task.id,
                error: err instanceof Error ? err.message : String(err),
              });
            });
  ...
  private transitionTaskState(
    ...
    try {
      ...
      // Reset processStartedAt when transitioning into busy/starting from a non-active state
      if (
        (newState === 'busy' || newState === 'starting') &&
        oldState !== 'busy' &&
        oldState !== 'starting'
      ) {
        task.processStartedAt = new Date();
  ...
  private async autoReconnectTasks(): Promise<void> {
    ...
    const tasksToReconnect = disconnectedIds.filter((id) => {
      ...
      if (now - lastActive > MAX_STALE_AGE_MS) {
        console.log(
          `[TaskSpawner] Skipping stale task ${id} (last active ${Math.round((now - lastActive) / 60000)}m ago), clearing shouldContinue`,
        );
    ...
    if (tasksToReconnect.length === 0) {
      console.log(
        `[TaskSpawner] No eligible tasks to auto-reconnect. ${disconnectedIds.length} tasks in lazy load.`,
      );
      this.autoReconnectPromise = null;
    ...
    console.log(
      `[TaskSpawner] Auto-reconnecting ${tasksToReconnect.length} tasks in batches of ${BATCH_SIZE}...`,
    );
    ...
    for (let i = 0; i < tasksToReconnect.length; i++) {
      ...
      console.log(
        `[TaskSpawner] Auto-reconnecting task ${i + 1}/${tasksToReconnect.length} (batch ${batchNum}/${totalBatches}): ${taskId}`,
      );
    ...
    if (failedTasks.length > 0) {
      console.log(
        `[TaskSpawner] Auto-reconnect complete. ${failedTasks.length}/${tasksToReconnect.length} task(s) failed to reconnect.`,
      );
    } else {
      console.log(
        `[TaskSpawner] Auto-reconnect complete. All ${tasksToReconnect.length} tasks reconnected successfully.`,
      );
    }
  }
  ...
  private loadPersistedTasks(): void {
    try {
      ...
      if (existsSync(this.persistencePath)) {
        ...
        console.log(
          `[TaskSpawner] Loading ${persistence.tasks.length} active tasks, ${persistence.archivedTasks?.length || 0} archived tasks`,
        );
        ...
        if (persistence.archivedTasks) {
          console.log(
            `[TaskSpawner] Loading ${persistence.archivedTasks.length} archived tasks (metadata only)`,
          );
          let migratedCount = 0;
          ...
          if (migratedCount > 0) {
            console.log(
              `[TaskSpawner] Migrated ${migratedCount} archived tasks to lazy loading format`,
            );
    ...
    try {
      ...
      if (this.fileModTimeOnLoad !== null && existsSync(this.persistencePath)) {
        ...
        if (currentStats.mtimeMs > this.fileModTimeOnLoad) {
          ...
          console.error(
            `[TaskSpawner]     Loaded at:  ${new Date(this.fileModTimeOnLoad).toISOString()}`,
          );
          console.error(
            `[TaskSpawner]     Modified at: ${new Date(currentStats.mtimeMs).toISOString()}`,
          );
      ...
      if (newTotal === 0 && existsSync(this.persistencePath)) {
        try {
          ...
          const existingTotal =
            (existing.tasks?.length || 0) + (existing.archivedTasks?.length || 0);
          if (existingTotal > 0) {
            console.error(
      ...
      atomicWriteFileSync(this.persistencePath, JSON.stringify(persistence, null, 2), {
        backup: true,
      });
      ...
      console.log(
        `[TaskSpawner] Saved ${tasksToSave.length} tasks, ${archivedTasksToSave.length} archived (metadata only)`,
      );
    } catch (error) {
  ...
  private cleanupMcpTempFiles(taskId: string): void {
    ...
    for (const filePath of filesToClean) {
      try {
        ...
        logger.warn('Failed to clean up MCP temp file', {
          filePath,
  ...
  private startReadyFallbackTimer(task: InternalTask): void {
    ...
    task.readyFallbackTimer = setTimeout(() => {
      if (!task.initialPromptSent && task.pendingPrompt) {
        console.log(
          `[TaskSpawner] FALLBACK: Ready signal not detected after ${READY_FALLBACK_MS}ms for task ${task.id}, sending prompt anyway`,
        );
        console.log(
          `[TaskSpawner] FALLBACK: Recent output: ${JSON.stringify(this.getRecentOutput(task, 512))}`,
        );
    ...
    if (hasQuestionMark) {
      ...
      for (const pattern of questionPatterns) {
        if (pattern.test(cleanSection)) {
          console.log(
            `[TaskSpawner] Question detected in last section: "${cleanSection.slice(0, 100)}..."`,
          );
          return 'question';
        }
      }
      ...
      if (trimmedSection.endsWith('?') && trimmedSection.length > 10) {
        console.log(
          `[TaskSpawner] Question detected (ends with ?): "${trimmedSection.slice(-80)}"`,
        );
        return 'question';
      }
      ...
      console.log(
        `[TaskSpawner] Has '?' but no question pattern matched. Section: "${trimmedSection.slice(0, 150)}"`,
      );
    }
  ...
  private sendPromptWithRetry(task: InternalTask, prompt: string, maxRetries = 5): void {
    ...
    console.log(
      `[TaskSpawner] Writing prompt to PTY: "${prompt.substring(0, 100)}${prompt.length > 100 ? '...' : ''}" (${prompt.length} chars, retries left: ${maxRetries})`,
    );
    ...
    if (prompt.length <= 200) {
      ...
      console.log(
        `[TaskSpawner] Waiting ${delayMs}ms before verifying prompt write for ${prompt.length} chars`,
      );
      ...
      setTimeout(() => {
        ...
        if (!outputGrew && maxRetries > 0) {
          console.log(
            `[TaskSpawner] Prompt write NOT accepted by TUI (outputDelta=${outputAfterWrite - outputBeforeWrite}), retrying in 1500ms (retries left: ${maxRetries - 1})`,
          );
        ...
        if (!outputGrew) {
          console.log(
            `[TaskSpawner] WARNING: Prompt write may not have been accepted (outputDelta=${outputAfterWrite - outputBeforeWrite}), but no retries left. Proceeding with Enter anyway.`,
          );
        }

        console.log(
          `[TaskSpawner] Prompt write verified (outputDelta=${outputAfterWrite - outputBeforeWrite}), sending Enter`,
        );
  ...
  private sendEnterWithRetry(
    ...
    console.log(
      `[TaskSpawner] Sending Enter for ${context} (attempt ${task.promptSubmitAttempts}) to task ${task.id}`,
    );
    ...
    const outputLengthBeforeEnter = options.outputLengthAtSend ?? task.totalOutputSize;
    ...
    setTimeout(() => {
      ...
      if (outputGrew || this.hasProcessingIndicators(task)) {
        console.log(
          `[TaskSpawner] Claude processing detected for ${context} after attempt ${task.promptSubmitAttempts} (outputGrew=${outputGrew}, outputDelta=${currentOutputLength - outputLengthBeforeEnter})`,
        );
      ...
      console.log(
        `[TaskSpawner] No processing indicators found for ${context} (outputDelta=${currentOutputLength - outputLengthBeforeEnter}), will retry Enter in 500ms`,
      );
      setTimeout(
        () =>
          this.sendEnterWithRetry(task, retriesLeft - 1, {
            ...options,
            outputLengthAtSend: outputLengthBeforeEnter,
          }),
        500,
      );
    }, 800);
  ...
  async createTask(
    ...
    const gitStatePromise = captureGitStateBefore(workspaceId).catch((err) => {
      logger.warn('Git state capture failed (non-blocking)', {
        error: err instanceof Error ? err.message : String(err),
      });
      return null;
    });
    ...
    if (this.configStore?.getUseLearnings() && this.learningsStore) {
      learningsPromise = this.learningsStore
        .searchLearnings({
    ...
    if (this.backendType === 'opencode' && this.backend) {
      ...
      task = await this.createTaskWithOpenCode(
        sanitizedPrompt,
        workspaceId,
      ...
      task = await this.createTaskWithClaudeCode(
        sanitizedPrompt,
        workspaceId,
        sanitizedSystemPrompt,
        gitStatePromise,
    ...
    learningsPromise
      .then((injectedLearnings) => {
        if (injectedLearnings.length > 0) {
          ...
          this.taskLearnings.set(
            task.id,
            injectedLearnings.map((l) => l.learning.id),
          );
        }
      })
      .catch((err) => {
        logger.warn('Learnings promise rejected (post-spawn)', {
          taskId: task.id,
          error: err instanceof Error ? err.message : String(err),
        });
      });
  ...
  private async createTaskWithOpenCode(
    ...
    if (gitStateBefore) {
      logger.info('Captured git state before task', {
        taskId: task.id,
        commit: gitStateBefore.commitBefore?.substring(0, 7),
      });
    }
...
`;
    ...
    logger.info(`Creating task with Claude Code`, {
      taskId: id,
      workspaceId,
      cols: initialCols,
      rows: initialRows,
    ...
    gitStatePromise
      .then((gitStateBefore) => {
        if (gitStateBefore) {
          ...
          logger.info(`Git state attached to task`, {
            taskId: id,
            commit: gitStateBefore.commitBefore?.substring(0, 7),
          });
        }
      ...
      .catch((err) => {
        logger.warn('Git state promise rejected (post-spawn)', {
          taskId: id,
          error: err instanceof Error ? err.message : String(err),
        });
      });
  ...
  private setupProcessHandlers(task: InternalTask): void {
    console.log(
      `[TaskSpawner] setupProcessHandlers: Setting up onData for task ${task.id}, pid=${task.process.pid}`,
    );
    ...
    task.process.onData((rawData: string) => {
      ...
      if (process.env.DEBUG_PTY === 'true') {
        ...
        console.log(
          `[TaskSpawner] PTY Output from ${task.id} (PID ${task.process.pid}): ${JSON.stringify(data.substring(0, 50))}`,
        );
      }
      ...
      if (
        ...
        task.pendingPrompt &&
        (this.isReadyForInitialInput(cleanData) ||
          this.isReadyForInitialInput(this.getRecentOutput(task, 4096)))
      ) {
        console.log(
          `[TaskSpawner] Claude ready, sending prompt (detected in ${this.isReadyForInitialInput(cleanData) ? 'current chunk' : 'accumulated output'})`,
        );
        task.initialPromptSent = true;
        const prompt = task.pendingPrompt;
        task.pendingPrompt = null;
      ...
      if (task.isActive) {
        ...
        if (!task.inactiveOutputLogged) {
          console.log(
            `[TaskSpawner] Output received for ${task.id} but isActive=false, dropping output (subsequent drops will be silent)`,
          );
    ...
    task.process.onExit(({ exitCode }) => {
      ...
      this.captureTokenUsage(task.id)
        .catch((err) => {
          logger.error('Unhandled error in captureTokenUsage (onExit)', {
            taskId: task.id,
            error: err instanceof Error ? err.message : String(err),
          });
        })
  ...
  async captureGitStateAfterTask(taskId: string): Promise<void> {
    ...
    try {
      ...
      this.scheduleSave();
      console.log(
        `[TaskSpawner] Git state after task: ${gitState.filesModified.length} files modified`,
      );
      this.emit('taskStateChanged', this.toPublicTask(task));
    } catch (error) {
  ...
  async revertTask(
    ...
    if (!gitState.canRevert) {
      return {
        success: false,
        error: 'Cannot revert: uncommitted changes existed before task',
        filesReverted: [],
      };
    }
  ...
  renameTask(taskId: string, displayName: string, source: 'user' | 'agent' = 'user'): boolean {
    ...
    if (task) {
      ...
      if (source === 'agent' && task.displayNameEditedByUser) {
        console.log(
          `[TaskSpawner] Agent rename blocked for task ${taskId} — title was edited by user`,
        );
        return false;
      }
    ...
    if (persisted) {
      if (source === 'agent' && persisted.displayNameEditedByUser) {
        console.log(
          `[TaskSpawner] Agent rename blocked for disconnected task ${taskId} — title was edited by user`,
        );
        return false;
      }
      ...
      this.scheduleSave();
      console.log(
        `[TaskSpawner] Renamed disconnected task ${taskId} to "${trimmed}" (source: ${source})`,
      );
      return true;
    }
    ...
    if (archived) {
      if (source === 'agent' && archived.displayNameEditedByUser) {
        console.log(
          `[TaskSpawner] Agent rename blocked for archived task ${taskId} — title was edited by user`,
        );
        return false;
      }
      ...
      this.scheduleSave();
      console.log(
        `[TaskSpawner] Renamed archived task ${taskId} to "${trimmed}" (source: ${source})`,
      );
      return true;
    }
  ...
  setTaskActive(taskId: string, active: boolean): void {
      ...
      setTimeout(() => {
        for (const task of this.tasks.values()) {
          ...
          if (task.id !== taskId && !task.isActive && !isRecentlyActive && task.previousHistory) {
            ...
            console.log(
              `[TaskSpawner] Freed memory for inactive task ${task.id} (last active ${Math.round(timeSinceLastActivity / 1000)}s ago)`,
            );
          }
        }
    ...
    console.log(
      `[TaskSpawner] setTaskActive called: taskId=${taskId}, active=${active}, inTasks=${this.tasks.has(taskId)}, inDisconnected=${this.disconnectedTasks.has(taskId)}`,
    );
    if (active && this.disconnectedTasks.has(taskId)) {
      // Auto-reconnect on select: spawn the Claude process so the task is
      // immediately ready for input when the user starts typing.
      console.log(
        `[TaskSpawner] Auto-reconnecting disconnected task ${taskId} on select`,
      );
      ...
      if (reconnectedTask) {
        ...
        // Reconnection failed — fall back to showing stored history
        console.warn(
          `[TaskSpawner] Failed to reconnect task ${taskId} on select, showing stored history`,
        );
    ...
    const task = this.tasks.get(taskId);
    console.log(
      `[TaskSpawner] setTaskActive: taskId=${taskId}, active=${active}, taskFound=${!!task}, currentIsActive=${task?.isActive}, ptyPid=${task?.process?.pid}`,
    );
    if (task) {
      ...
      console.warn(
        `[TaskSpawner] Task ${taskId} not found in tasks or disconnectedTasks, sending empty restore`,
      );
      this.emit('taskRestore', taskId, '');
    }
  }
        ...
        try {
          ...
          if (isRawText) {
            ...
            if (fileSize > MAX_HISTORY_TO_SEND) {
              ...
              console.log(
                `[TaskSpawner] Loaded tail of RAW history: ${fileSize} bytes -> ${MAX_HISTORY_TO_SEND} bytes`,
              );
            } else {
            ...
            if (fileSize > maxBase64Size) {
              ...
              console.log(
                `[TaskSpawner] Loaded tail of Base64 history from file for ${task.id}: ${fileSize} bytes (file) -> ${maxBase64Size} bytes (loaded)`,
              );
            } else {
              // File is small enough, read it all
              base64Content = readFileSync(historyPath, 'utf-8');
              console.log(
                `[TaskSpawner] Loaded complete Base64 history from file for ${task.id}: ${fileSize} bytes`,
              );
            }
          }
    ...
    if (task.lazyHistoryBase64 && !task.previousHistory) {
      try {
        ...
        if (fullHistory.length > MAX_HISTORY_TO_SEND) {
          ...
          console.log(
            `[TaskSpawner] Lazy loaded and truncated history from ${fullHistory.length} to ${task.previousHistory.length} bytes`,
          );
        } else {
  ...
  private async renderConversationAsTerminal(
    workspaceId: string,
    sessionId: string,
  ): Promise<string | null> {
    try {
  ...
  writeToTask(taskId: string, data: string): void {
    ...
    if (!task) {
      if (this.disconnectedTasks.has(taskId)) {
        ...
        console.log(
          `[TaskSpawner] Input length: ${data.length}, Data preview: ${JSON.stringify(data.substring(0, 50))}`,
        );
        const reconnectedTask = this.reconnectTask(taskId);
        if (reconnectedTask) {
          ...
          if (task) {
            ...
            const onStateChanged = (changedTask: Task) => {
              if (changedTask.id === taskId && changedTask.state === 'idle' && !delivered) {
                ...
                console.log(
                  `[TaskSpawner] Task ${taskId} reached idle after reconnect, delivering pending write`,
                );
            ...
            setTimeout(() => {
              if (!delivered) {
                ...
                this.removeListener('taskStateChanged', onStateChanged);
                console.log(
                  `[TaskSpawner] Fallback: delivering pending write after 15s for task ${taskId}`,
                );
                this.writeToTask(taskId, data);
              }
            }, 15000);
    ...
    if (hasMessageContent && (task.state === 'idle' || task.state === 'waiting_input')) {
      ...
      console.log(
        `[TaskSpawner] Writing message to task ${taskId} (${messageContent.length} chars), sending Enter in ${enterDelayMs}ms`,
      );
      ...
      setTimeout(
        () => this.sendEnterWithRetry(task, 5, { isInitialPrompt: false, enterKey }),
        enterDelayMs,
      );
        ...
        if (newTask) {
          ...
          const onStateChanged = (changedTask: Task) => {
            if (changedTask.id === taskId && changedTask.state === 'idle' && !delivered) {
              ...
              this.removeListener('taskStateChanged', onStateChanged);
              console.log(
                `[TaskSpawner] Task ${taskId} reached idle after exited-reconnect, delivering pending write`,
              );
              this.writeToTask(taskId, data);
            }
          };
          ...
          setTimeout(() => {
            if (!delivered) {
              ...
              this.removeListener('taskStateChanged', onStateChanged);
              console.log(
                `[TaskSpawner] Fallback: delivering pending write after 15s for exited-reconnect ${taskId}`,
              );
              this.writeToTask(taskId, data);
            }
          }, 15000);
  ...
  stopTask(taskId: string): boolean {
    ...
    if (!task) {
      ...
      const disconnected = this.disconnectedTasks.get(taskId);
      if (disconnected) {
        logger.info(`stopTask: task is disconnected, transitioning to disconnected state`, {
          taskId,
        });
        // Clear stale flags
  ...
  archiveTask(taskId: string): void {
    ...
    if (task) {
      ...
      if (task.state === 'busy' || task.state === 'starting' || task.state === 'waiting_input') {
        logger.info(`Task is running, sending interrupt before archive`, {
          taskId,
          state: task.state,
        });
        try {
    ...
    if (disconnected) {
      ...
      if (disconnected.outputHistory) {
        try {
          ...
          console.log(
            `[TaskSpawner] Saved disconnected task history to disk: ${historySize} bytes`,
          );
        } catch (e) {
          console.error(`[TaskSpawner] Failed to save archived history:`, e);
        }
      ...
      const archivedMetadata: ArchivedTaskMetadata = {
        ...
        historySize: disconnected.outputHistory
          ? Math.floor(disconnected.outputHistory.length * 0.75)
          : 0,
        displayName: disconnected.displayName,
        displayNameEditedByUser: disconnected.displayNameEditedByUser,
      };
  ...
  restoreArchivedTask(taskId: string): Task | null {
    ...
    if (existsSync(historyPath)) {
      try {
        ...
        console.log(
          `[TaskSpawner] Loaded archived history from disk: ${archived.historySize || 0} bytes`,
        );
      } catch (e) {
  ...
  reconnectTask(taskId: string): Task | null {
    ...
    if (this.tasks.has(taskId)) {
      ...
      if (existing.state !== 'exited') {
        console.log(
          `[TaskSpawner] Task ${taskId} is already live (state=${existing.state}), skipping reconnect`,
        );
    ...
    if (taskBackendType === 'opencode') {
      ...
      if (sessionIdToUse) {
        ...
        console.log(
          `[TaskSpawner] Reconnecting OpenCode task ${taskId} with session ${sessionIdToUse}`,
        );
      } else {
      ...
      if (sessionIdToUse) {
        ...
        console.log(
          `[TaskSpawner] Reconnecting Claude Code task ${taskId} with session ${sessionIdToUse}`,
        );
      } else {
    ...
    try {
      ...
      if (existsSync(historyPath)) {
        ...
        if (stat.size <= MAX_RECONNECT_HISTORY) {
          ...
          previousHistory = buf;
          console.log(
            `[TaskSpawner] Large history file (${stat.size} bytes), loaded tail ${MAX_RECONNECT_HISTORY} bytes for ${taskId}`,
          );
        }
        console.log(
          `[TaskSpawner] Preserved ${previousHistory.length} bytes of history for ${taskId} on reconnect`,
        );
      }
    } catch (e) {
    ...
    if (!previousHistory && persisted.outputHistory) {
      try {
        ...
        console.log(
          `[TaskSpawner] Preserved ${previousHistory.length} bytes of in-memory history for ${taskId}`,
        );
      } catch (e) {
    ...
    const task: InternalTask = {
      ...
      processStartedAt: shouldContinue
        ? persisted.processStartedAt
          ? new Date(persisted.processStartedAt)
          : now
    ...
    console.log(
      `[TaskSpawner] reconnectTask: Setting up process handlers for ${task.id}, ptyPid=${ptyProcess.pid}`,
    );
    this.setupProcessHandlers(task);
    this.tasks.set(task.id, task);
    this.taskBackends.set(task.id, taskBackendType);
    console.log(
      `[TaskSpawner] reconnectTask: Task ${task.id} added to tasks map (backend: ${taskBackendType})`,
    );
    ...
    if (pendingResize) {
      try {
        ...
        console.log(
          `[TaskSpawner] Applied pending resize for ${taskId}: ${pendingResize.cols}x${pendingResize.rows}`,
        );
      } catch (e) {
