...
`;
    ...
    private syncIsActiveFlag(): void {
        for (const task of this.tasks.values()) {
            ...
            if (task.isActive !== visible) {
                ...
                // An OpenCode task keeps its own isActive inside the backend
                // adapter, which gates its output. Tell it when the task leaves
                // the visible set (a pane closed, a setVisible prune, a client
                // disconnect); becoming visible is handled by setTaskActive(),
                // which is also where history is replayed.
                if (!visible && this.taskBackends.get(task.id) === 'opencode' && this.backend) {
                    this.backend.setTaskActive(task.id, false);
                }
            }
        }
    }
    ...
    setTaskActive(taskId: string, active: boolean, clientId = '__server__'): void {
        ...
        if (task) {
            ...
            logger.debug(`Set task.isActive to ${task.isActive} for ${taskId} (requested=${active})`);
