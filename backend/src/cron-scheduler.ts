...
function cronMatchesDate(cron: ParsedCron, date: Date): boolean {
  ...
  if (domConstrained && dowConstrained) {
    return (
      fieldMatches(cron.dayOfMonth, dayOfMonth) || fieldMatches(cron.dayOfWeek, dayOfWeek, true)
    );
  }
...
export class CronScheduler extends EventEmitter {
  ...
  private check(): void {
    ...
    for (const [id, scheduled] of this.scheduledTasks.entries()) {
      ...
      const taskState = this.taskStateChecker(scheduled.taskId);
      if (taskState === 'exited') {
        logger.info('Removing scheduled task (parent task exited)', {
          id,
      ...
      if (taskState === 'unknown') {
        logger.debug('Skipping scheduled task check (task state unknown, may be reconnecting)', {
          id,
          taskId: scheduled.taskId,
        });
        continue;
      ...
      if (cronMatchesDate(cron, now)) {
        logger.info('Scheduled task due', {
          id,
          taskId: scheduled.taskId,
          prompt: scheduled.prompt.substring(0, 80),
        });
