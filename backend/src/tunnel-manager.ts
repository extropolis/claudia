...
export class TunnelManager extends EventEmitter {
  ...
  async start(): Promise<TunnelStatus> {
    if (this.ngrokProcess || this.adoptedMonitor) {
      logger.info('Tunnel already active', {
        url: this.url,
        adopted: this.adoptedMonitor !== null,
      });
    ...
    try {
      ...
      logger.warn('Failed to fetch public IP', {
    ...
    if (existingUrl) {
      logger.info('Found existing ngrok tunnel — adopting it (keeps same URL, no re-scan needed)', {
        url: existingUrl,
      });
  ...
  private async startNgrok(): Promise<void> {
    ...
    try {
      execSync(`${process.platform === 'win32' ? 'where' : 'which'} ${ngrokExe}`, {
        stdio: 'ignore',
      });
    } catch {
      throw new Error(
        `ngrok is not installed. Install it from https://ngrok.com/download and ensure it's in your PATH.`,
      );
    }
    ...
    const earlyExitPromise = new Promise<never>((_resolve, reject) => {
      ngrok.on('close', (code) => {
        logger.info('Ngrok process exited during startup', {
          code,
          output: startupOutput.join('\n'),
        });
        ...
        if (!this.stopping) {
          const errorDetail =
            startupOutput.length > 0 ? startupOutput.join(' | ') : `exit code ${code}`;
    ...
    let url: string | null;
    try {
      url = await Promise.race([this.pollNgrokApi(15000), earlyExitPromise]);
    } catch (err) {
      ngrok.kill();
      throw err;
    }
    ...
    if (!url) {
      ngrok.kill();
      const errorDetail =
        startupOutput.length > 0 ? startupOutput.join(' | ') : 'no response from ngrok local API';
    ...
    if (this.ngrokProcess) {
      try {
        if (process.platform === 'win32' && this.ngrokProcess.pid) {
          ...
          execSync(`taskkill /F /T /PID ${this.ngrokProcess.pid} 2>nul || exit /b 0`, {
            stdio: 'ignore',
          });
        } else {
        ...
        logger.error('Error stopping ngrok', {
