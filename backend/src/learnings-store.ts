...
export class LearningsStore {
  ...
  async generateEmbedding(text: string): Promise<number[]> {
    ...
    console.log(
      `[LearningsStore] Generated embedding with ${data.data[0].embedding.length} dimensions`,
    );
  ...
  async searchLearnings(params: {
    ...
    const { query, workspaceId, topK = 5, minScore = 0.3, useUtilityRanking = true } = params;
  ...
  updateUtility(id: string, success: boolean, learningRate: number = 0.1): void {
    ...
    this.saveData();
    console.log(
      `[LearningsStore] Updated utility for ${id}: ${learning.utility.toFixed(3)} (${success ? 'success' : 'failure'})`,
    );
  }
  ...
  formatForContext(learnings: LearningSearchResult[]): string {
    ...
    for (const { learning, score } of learnings) {
      ...
      lines.push(
        `(Relevance: ${(score * 100).toFixed(0)}%, Utility: ${(learning.utility * 100).toFixed(0)}%)`,
      );
      lines.push('');
    }
