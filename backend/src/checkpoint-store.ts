...
export class CheckpointStore {
  ...
  constructor(basePath?: string) {
    ...
    console.log(
      `[CheckpointStore] Loaded ${this.data.checkpoints.length} checkpoints from ${this.filePath}`,
    );
  }
  ...
  async createCheckpoint(
    ...
    const autoName =
      name ||
      `Checkpoint ${new Date().toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}`;
    ...
    console.log(
      `[CheckpointStore] Created checkpoint "${checkpoint.name}" for task ${taskId} (git: ${gitRef?.substring(0, 7) || 'n/a'}, branch: ${gitBranch || 'n/a'})`,
    );
    return checkpoint;
  }
  ...
  async restoreCheckpoint(checkpointId: string): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      ...
      console.log(
        `[CheckpointStore] Restoring checkpoint "${checkpoint.name}" - stashing current changes...`,
      );
      try {
      ...
      if (checkpoint.gitDiff) {
        ...
        try {
          ...
          console.warn(
            '[CheckpointStore] Could not apply saved diff (may have conflicts):',
            applyErr,
          );
  ...
  async forkFromCheckpoint(
    ...
    try {
      ...
      console.log(
        `[CheckpointStore] Forking from checkpoint "${checkpoint.name}" to branch "${branch}"...`,
      );
  ...
  async restoreCheckpointSelective(checkpointId: string): Promise<{
    ...
    if (!checkpoint.gitRef) {
      return {
        ...
        restoredFiles: [],
        conflictingFiles: [],
        error: 'Checkpoint has no git reference',
      };
    }
    ...
    try {
      const { stdout: currentDiff } = await execAsync('git diff HEAD', {
        cwd,
        maxBuffer: 1024 * 1024,
      });
      ...
      console.log(
        `[CheckpointStore] Selective restore: ${restoredFiles.length} restored, ${conflictingFiles.length} conflicts`,
      );
      return { success: true, restoredFiles, conflictingFiles };
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
