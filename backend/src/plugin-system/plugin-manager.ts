...
export class PluginManager {
    ...
    registerRoutes(app: Router): void {
        ...
        // First pass: register ai-provider plugins at root if they match current apiMode
        const currentApiMode = this.context.configStore.getApiMode();
        for (const [name, plugin] of this.plugins.entries()) {
            if (plugin.manifest.type === 'ai-provider' &&
                plugin.manifest.backend?.provides?.apiMode === currentApiMode &&
                plugin.getRouter) {
                try {
                    const router = plugin.getRouter();
                    app.use('/', router);
                    logger.info(`Registered AI provider routes for ${name} at /`);
                } catch (error) {
