...
export async function parseSessionTokenUsage(
    ...
    return new Promise((resolve, reject) => {
        ...
        const assistantUsageByUuid = new Map<string, { model: string; usage: JsonlUsage; costUSD?: number }>();
        ...
        rl.on('close', () => {
            if (parseErrorCount > 0) {
                console.log(`[TokenParser] Skipped ${parseErrorCount}/${lineCount} malformed lines in ${sessionFilePath}`);
            }
            ...
            console.log(`[TokenParser] Parsed ${sessionFilePath}: ${assistantUsageByUuid.size} assistant turns, ${toolResultUsageByUuid.size} tool results, ${totalInputTokens} in / ${totalOutputTokens} out, cost $${totalCostUsd.toFixed(4)}`);
