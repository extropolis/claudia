...
import { Task, Workspace, WorkspaceReference, WSMessage, WSMessageType, WSErrorPayload, ChatMessage, SuggestedAction, WaitingInputType, ScheduledTask, Checkpoint, PORTS, TaskTokenUsage, UsageDashboardData, TaskWorkStatus, DeleteRequestPayload } from '@claudia/shared';
            ...
            try {
                ...
                switch (message.type) {
                    ...
                    case 'task:deleteRequest': {
                        ...
                        const { requestId, tasks } = payload as Partial<DeleteRequestPayload>;
                    ...
                    case 'task:deleteResolved': {
                        ...
                        // archiveTask() is fire-and-forget: it logs and returns for
                        // an id it does not know. Reporting that as "deleted" would
                        // have the agent tell the user a task went when it did not,
                        // so an unknown id is checked for up front and reported as a
                        // failure instead.
                        const known = new Set(taskSpawner.getAllTasks().map(t => t.id));
                        const archived: string[] = [];
                        const failed: Array<{ taskId: string; reason: string }> = [];
                        for (const taskId of approvedIds) {
                            if (!known.has(taskId)) {
                                logger.warn('Bulk delete skipped an unknown task', { taskId, requestId });
