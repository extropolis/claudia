...
export async function captureGitStateBefore(cwd: string): Promise<Partial<TaskGitState> | null> {
  ...
  console.log(
    `[GitUtils] Captured before state: commit=${commitBefore.substring(0, 7)}, uncommitted=${uncommittedBefore}`,
  );
...
export async function captureGitStateAfter(
  ...
  console.log(
    `[GitUtils] Captured after state: commit=${commitAfter?.substring(0, 7)}, files=${filesModified.length}, canRevert=${canRevert}`,
  );
  ...
  try {
    ...
    if (gitState.commitBefore !== currentHead) {
      ...
      if (commitsBehind > MAX_REVERT_COMMITS) {
        return {
          success: false,
          error:
            `Revert blocked: would undo ${commitsBehind} commits (max ${MAX_REVERT_COMMITS}). ` +
    ...
    if (currentHead !== gitState.commitBefore) {
      if (!isValidCommitHash(gitState.commitBefore)) {
        return {
          success: false,
          error: 'Invalid commit hash in stored git state',
          filesReverted: [],
        };
      }
      console.log(
        `[GitUtils] Resetting to commit ${gitState.commitBefore.substring(0, 7)} (undoing ${await countCommitsBetween(cwd, gitState.commitBefore, currentHead)} commits)`,
      );
