...
export function detectWaitingForInput(str: string): WaitingInputType | null {
  ...
  if (hasQuestionMark) {
    ...
    if (trimmedSection.endsWith('?') && trimmedSection.length > 10) {
      console.log(
        `[StateDetection] Question detected (ends with ?): "${trimmedSection.slice(-80)}"`,
      );
      return 'question';
    }

    console.log(
      `[StateDetection] Has '?' but no question pattern matched. Section: "${trimmedSection.slice(0, 150)}"`,
    );
  }
