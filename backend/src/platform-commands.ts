...
export function buildReferenceFolderPickerCommand(platform: Platform): CommandSpec {
  if (platform === 'darwin') {
    ...
    return {
      ...
      args: [
        '-Command',
        `Add-Type -AssemblyName System.Windows.Forms; $f = New-Object System.Windows.Forms.FolderBrowserDialog; $f.Description = 'Select reference folder'; if ($f.ShowDialog() -eq 'OK') { $f.SelectedPath } else { '' }`,
      ],
    };
  } else {
    return {
 ...
 */
export function buildPathWithExtras(
  platform: Platform,
  homeDir: string,
  originalPath: string,
): string {
...
export function buildAppMenuEntries(
  platform: Platform,
  appName: string,
): Array<{ label: string; submenu: Array<{ role?: string; type?: string }> }> {
  if (platform === 'darwin') {
    return [
      {
        label: appName,
