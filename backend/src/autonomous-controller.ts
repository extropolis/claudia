...
export class AutonomousController extends EventEmitter {
  ...
  async start(goal: string, workspacePath: string, maxIterations?: number): Promise<string> {
    ...
    console.log(
      `[Autonomous] Starting session ${this.session.id} — goal: "${goal}" workspace: ${workspacePath}`,
    );
  ...
  isActive(): boolean {
    return this.session !== null && !['idle', 'complete', 'failed'].includes(this.session.state);
  }
  ...
  private async processLoop(): Promise<void> {
    ...
    for (const step of inProgressSteps) {
      ...
      if (taskState === 'idle' || taskState === 'exited') {
        ...
        console.log(
          `[Autonomous] Step "${step.description}" task ${step.taskId} completed (state: ${taskState})`,
        );
    ...
    const needsTesting = this.session.plan.find(
      (s) => s.status === 'completed' && s.testRequired && !this.hasTestResult(s.id),
    );
    if (needsTesting) {
      this.session.state = 'testing';
      this.emitStateChange();
    ...
    const allDone = this.session.plan.every(
    ...
    if (allDone && this.session.plan.length > 0) {
      ...
      if (failedCount === 0) {
        ...
        this.emitStateChange();
        this.announce(
          `Finished with ${failedCount} failed step(s) out of ${this.session.plan.length}.`,
        );
      }
      return;
    }
  ...
  private async generatePlan(): Promise<PlanStep[]> {
    ...
    const adjustments =
      this.session.goalAdjustments.length > 0
...
Respond with ONLY valid JSON (no markdown code fences):
{
  "steps": [
    ...
    const systemPrompt =
      'You are a software development planner. Generate a step-by-step plan to achieve the given goal. Respond with ONLY valid JSON.';
  ...
  private async spawnStepTask(step: PlanStep): Promise<void> {
    ...
    try {
      const task = await this.taskSpawner.createTask(step.taskPrompt, this.session.workspaceId);
      step.taskId = task.id;
  ...
  private async evaluateStep(step: PlanStep): Promise<void> {
    ...
    if (assessment.success) {
      ...
      if (step.retries <= step.maxRetries) {
        ...
        console.log(
          `[Autonomous] Step "${step.description}" failed (retry ${step.retries}/${step.maxRetries}): ${assessment.summary}`,
        );
        step.status = 'pending';
        step.taskPrompt = this.augmentPromptWithError(step.taskPrompt, assessment.summary);
        step.taskId = undefined;
        this.announce(
          `Retrying: ${step.description}. Error was: ${assessment.summary.substring(0, 60)}`,
        );
      } else {
        step.status = 'failed';
        step.result = assessment.summary;
        this.session.consecutiveFailures++;
        console.log(
          `[Autonomous] Step "${step.description}" FAILED permanently: ${assessment.summary}`,
        );
  ...
  private async assessTaskOutput(taskId: string): Promise<{ success: boolean; summary: string }> {
    try {
      ...
      if (output.length > 100) {
        try {
          const systemPrompt =
            'You assess whether coding tasks completed successfully. Respond with ONLY valid JSON.';
      ...
      if (hasError) {
        ...
        return {
          success: false,
          summary: errorLine.substring(0, 200) || 'Task output contains errors',
        };
      }
  ...
  private async runTestsForStep(step: PlanStep): Promise<void> {
    ...
    if (!buildResult.passed) {
      ...
      if (step.retries <= step.maxRetries) {
        step.status = 'pending';
        step.taskPrompt = this.augmentPromptWithError(
          step.taskPrompt,
          `Build failed: ${buildResult.details}`,
        );
        step.taskId = undefined;
      } else {
        step.status = 'failed';
...
At the end output exactly "RESULT: PASS" if all checks pass, or "RESULT: FAIL" with specific details about what failed.`,
        ...
        if (!playwrightResult.passed) {
          ...
          if (step.retries <= step.maxRetries) {
            ...
            step.taskPrompt = this.augmentPromptWithError(
              step.taskPrompt,
              `Playwright test failed: ${playwrightResult.details}`,
            );
            step.taskId = undefined;
          } else {
            step.status = 'failed';
  ...
  private async spawnTestTask(
    prompt: string,
    type: TestResult['type'],
  ): Promise<Omit<TestResult, 'stepId' | 'timestamp'>> {
    ...
    try {
      ...
      const failDetails = output.includes('RESULT: FAIL')
        ...
        : passed
          ? ''
          : 'No explicit PASS/FAIL result found';
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (portMatch) {
          ...
          console.log(
            `[Autonomous] Dev server port not detected, assuming ${this.session.devServerPort}`,
          );
        }
      }
    } catch (err) {
  ...
  private checkFailureLimit(): void {
    ...
    if (this.session.consecutiveFailures >= this.session.maxConsecutiveFailures) {
      console.log(
        `[Autonomous] Max consecutive failures reached (${this.session.maxConsecutiveFailures})`,
      );
      this.session.state = 'failed';
      this.stopTickLoop();
      this.emitStateChange();
