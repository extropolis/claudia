...
server.tool(
    ...
    `Schedule a recurring or one-shot prompt for a task. The prompt will be sent to the task's terminal at the scheduled time. Uses standard 5-field cron expressions (minute hour day-of-month month day-of-week). Examples: "*/5 * * * *" = every 5 minutes, "0 * * * *" = every hour, "0 9 * * 1-5" = weekdays at 9am. Recurring tasks auto-expire after 3 days. Max 50 scheduled tasks per task.`,
    {
        taskId: z.string().describe(`The task ID to schedule a prompt for.${SELF_TASK_ID ? ` To schedule on your own task, use "${SELF_TASK_ID}".` : ''}`),
        cronExpression: z.string().describe('5-field cron expression: "minute hour day-of-month month day-of-week". Examples: "*/5 * * * *" (every 5 min), "0 9 * * *" (daily 9am), "30 14 * * 1-5" (weekdays 2:30pm)'),
        prompt: z.string().describe('The prompt to send when the schedule fires'),
        isRecurring: z.boolean().optional().describe('true (default) for recurring, false for one-shot (fires once then deletes itself)'),
    },
    async ({ taskId, cronExpression, prompt, isRecurring }) => {
        try {
            log.info(`Creating cron: task=${taskId} expr="${cronExpression}" recurring=${isRecurring !== false}`);
            const result = await sendWSMessage('cron:create', {
                taskId, cronExpression, prompt, isRecurring: isRecurring !== false,
            });
            return {
                content: [{ type: 'text', text: JSON.stringify({
                    success: true,
                    scheduledTask: (result as any)?.scheduledTask,
                    description: (result as any)?.description,
                    message: `Scheduled task created. Prompt will be sent on cron "${cronExpression}".`,
                }, null, 2) }]
            };
        } catch (error) {
            log.error('Failed to create cron:', error);
...
server.tool(
    ...
    async ({ taskId }) => {
        try {
            const result = await sendWSMessage('cron:list', { taskId });
            const scheduled = (result as any)?.scheduledTasks || [];
            return {
                content: [{ type: 'text', text: JSON.stringify({
            ...
            log.error('Failed to list cron:', error);
            return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: String(error) }) }] };
        }
    }
);
...
server.tool(
    ...
    async ({ cronId }) => {
        try {
            log.info(`Deleting cron: ${cronId}`);
            await sendWSMessage('cron:delete', { cronId });
            return {
                content: [{ type: 'text', text: JSON.stringify({ success: true, message: `Scheduled task '${cronId}' deleted.` }) }]
            };
        } catch (error) {
            log.error('Failed to delete cron:', error);
            return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: String(error) }) }] };
        }
    }
);
...
server.tool(
    ...
    async ({ cronId, paused }) => {
        try {
            log.info(`${paused ? 'Pausing' : 'Resuming'} cron: ${cronId}`);
            await sendWSMessage('cron:update', { cronId, isPaused: paused });
            return {
                content: [{ type: 'text', text: JSON.stringify({ success: true, message: `Scheduled task '${cronId}' ${paused ? 'paused' : 'resumed'}.` }) }]
            };
        } catch (error) {
            log.error(`Failed to ${paused ? 'pause' : 'resume'} cron:`, error);
            return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: String(error) }) }] };
        }
    }
);
