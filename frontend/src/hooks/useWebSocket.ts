...
export function sendWsMessage(type: string, payload: unknown): void {
  if (_activeWs?.readyState === WebSocket.OPEN) {
    ...
    console.warn(
      `[WebSocket] sendWsMessage: cannot send ${type} - WS not open (state: ${_activeWs?.readyState})`,
    );
  }
}
...
export function useWebSocket() {
  ...
  const connect = useCallback(async () => {
    ...
    console.log(
      `[WebSocket] connect() called - current state: ${currentState}, isTunnel: ${isTunnelAccess()}`,
    );
    ...
    ws.onclose = (event) => {
      console.log(
        `[WebSocket] ❌ DISCONNECTED - code: ${event.code}, reason: ${event.reason || 'none'}, wasClean: ${event.wasClean}`,
      );
      ...
      console.log(
        `[WebSocket] ⏱️ Scheduling reconnect in ${delay}ms (attempt ${reconnectAttempts.current + 1})`,
      );
        ...
        switch (message.type) {
          case 'init': {
            ...
            fetch(`${API_URL}/api/config`)
              ...
              .then((config) => {
                ...
                const aiCoreConfigured =
                  config.aiCoreConfiguredFromEnv ||
                  !!(
                ...
                if (frontendDgKey && frontendDgKey !== config.deepgramApiKey) {
                  fetch(`${API_URL}/api/config`, {
                    ...
                    console.error('[WebSocket] Failed to sync Deepgram key to backend:', err),
                  );
                }
              })
          ...
          case 'task:waitingInput': {
            ...
            // Log to activity feed; only mark unread if not currently viewing
            {
              ...
              const inputLabel =
                payload.inputType === 'permission'
                  ? 'Needs permission'
                  : payload.inputType === 'question'
                    ? 'Has a question'
                    : 'Needs confirmation';
            ...
            if (autoFocusOnInput && selectedTaskId !== payload.taskId) {
              ...
              setTimeout(() => {
                window.dispatchEvent(
                  new CustomEvent('taskInput:focus', {
                    detail: { taskId: payload.taskId },
                  }),
                );
              }, 150);
          ...
          case 'task:stateChanged': {
            ...
            console.log(
              '[WebSocket] task:stateChanged received:',
              payload.task?.id,
              'state:',
              payload.task?.state,
            );
          ...
          case 'task:tokenUsage': {
            const payload = message.payload as {
              taskId: string;
              tokenUsage: import('@claudia/shared').TaskTokenUsage;
            };
          ...
          case 'cron:fired': {
            ...
            console.log(
              `[WebSocket] Scheduled task fired: ${payload.scheduledTaskId} → task ${payload.taskId}`,
            );
            break;
          }
          case 'tunnel:status': {
  ...
  const sendMessage = useCallback((type: string, payload: unknown) => {
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      ...
      console.warn(
        `[WebSocket] Cannot send ${type}: WebSocket not open (state: ${wsRef.current?.readyState})`,
      );
    }
  }, []);
  ...
  const createTask = useCallback(
    (prompt: string, workspaceId: string, initialCols?: number, initialRows?: number) => {
  ...
  const revertTask = useCallback(
    (taskId: string, cleanUntracked: boolean = false) => {
  ...
  const setSystemPrompt = useCallback(
    (workspaceId: string, systemPrompt: string) => {
  ...
  const setPreviewPort = useCallback(
    (workspaceId: string, port: number | undefined) => {
  ...
  const clearRecentWorkspace = useCallback(
    (workspaceId?: string) => {
  ...
  const continueArchivedTask = useCallback(
    (taskId: string) => {
  ...
  const renameTask = useCallback(
    (taskId: string, displayName: string) => {
  ...
  const toggleReference = useCallback(
    (workspaceId: string, referencePath: string) => {
      sendMessage('workspace:references:toggle', { workspaceId, referencePath });
    },
    [sendMessage],
  );

  const addCustomReference = useCallback(
    (workspaceId: string, path: string, description?: string) => {
  ...
  const removeReference = useCallback(
    (workspaceId: string, referenceId: string) => {
  ...
  const createScheduledTask = useCallback(
    (taskId: string, cronExpression: string, prompt: string, isRecurring: boolean = true) => {
