...
export function useVoiceRecognition(options: VoiceRecognitionOptions = {}) {
  ...
  useEffect(() => {
    ...
    recognition.onend = () => {
      console.log(
        '[VoiceRecognition] onend fired | shouldBeListening:',
        shouldBeListeningRef.current,
        '| continuous:',
        continuous,
      );
      ...
      if (shouldBeListeningRef.current && continuous) {
        ...
        try {
          ...
          restartAttempts++;
          console.warn(
            '[VoiceRecognition] Restart failed, attempt:',
            restartAttempts,
            '| error:',
            e,
          );
          ...
          setTimeout(() => {
            if (shouldBeListeningRef.current && recognitionRef.current) {
              try {
                ...
                restartAttempts++;
                console.error(
                  '[VoiceRecognition] Delayed restart failed, attempt:',
                  restartAttempts,
                  '| error:',
                  retryError,
                );
                if (restartAttempts >= maxRestartAttempts) {
                  ...
                  console.error(
                    '[VoiceRecognition] Max restart attempts reached after delay, giving up',
                  );
                  setIsListening(false);
                  shouldBeListeningRef.current = false;
                  onListeningChangeRef.current?.(false);
        ...
        console.log(
          '[VoiceRecognition] Not restarting (shouldBeListening:',
          shouldBeListeningRef.current,
          ', continuous:',
          continuous,
          ')',
  ...
  const startListening = useCallback(() => {
    console.log(
      '[VoiceRecognition] startListening called | isListening:',
      isListening,
      '| shouldBeListening:',
      shouldBeListeningRef.current,
      '| hasRecognition:',
  ...
  const stopListening = useCallback(() => {
    console.log(
      '[VoiceRecognition] stopListening called | hasRecognition:',
      !!recognitionRef.current,
    );
