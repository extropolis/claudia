...
export function useDeepgramRecognition(options: DeepgramRecognitionOptions = {}) {
    ...
    const startListening = useCallback(async () => {
        console.log('[DeepgramRecognition] startListening called | isListening:', isListening, '| shouldBeListening:', shouldBeListeningRef.current);
        ...
        try {
            ...
            ws.onopen = () => {
                ...
                try {
                    ...
                    console.log('[DeepgramRecognition] MediaRecorder started (250ms chunks, mimeType:', mimeType || 'default', ')');
            ...
            ws.onmessage = (event) => {
                try {
                    ...
                    if (data.type === 'Results') {
                        ...
                        console.log('[DeepgramRecognition] Transcript:', text, '| is_final:', isFinal, '| speech_final:', speechFinal);
            ...
            ws.onclose = (event) => {
                console.log('[DeepgramRecognition] WebSocket closed:', event.code, event.reason, '| shouldBeListening:', shouldBeListeningRef.current);
     ...
     */
    const reconnect = useCallback((stream: MediaStream) => {
        ...
        ws.onclose = (event) => {
            console.log('[DeepgramRecognition] Reconnect WS closed:', event.code, '| shouldBeListening:', shouldBeListeningRef.current);
