...
export const useTaskStore = create<TaskStore>()(
  persist(
    (set, get) => ({
      ...
      setErrorNotification: (message, code) =>
        set({ errorNotification: { message, code, timestamp: new Date() } }),
        ...
        if (id) {
          ...
          if (task) {
            ...
            if (unreadTaskIds.has(id)) {
              ...
              set({
                selectedTaskId: id,
                lastSelectedTaskByWorkspace: newMap,
                unreadTaskIds: newUnread,
              });
            } else {
      ...
      setTasks: (tasks) => {
        ...
        for (const task of tasks) {
          ...
          if (existing) {
            ...
            if (existingTime > incomingTime) {
              console.log(
                `[TaskStore] Keeping existing task ${task.id} (local: ${existingTime}, incoming: ${incomingTime})`,
              );
              taskMap.set(task.id, existing);
            } else {
              // Preserve existing order if incoming task doesn't have one
              const mergedTask =
                task.order === undefined && existing.order !== undefined
        ...
        const newSelectedId =
          selectedTaskId && !taskMap.has(selectedTaskId) ? null : selectedTaskId;
      ...
      updateTask: (task) => {
        ...
        if (existing) {
          ...
          if (existingTime > incomingTime) {
            console.log(
              `[TaskStore] Skipping older update for task ${task.id} (local: ${existingTime}, incoming: ${incomingTime})`,
            );
            return;
          }
        ...
        // Preserve existing order if incoming task doesn't have one
        const mergedTask =
          existing && task.order === undefined && existing.order !== undefined
            ? { ...task, order: existing.order }
            : task;
      ...
      setWorkspaces: (workspaces) => {
        const {
          expandedWorkspaces: currentExpanded,
      ...
      setElevenLabsVoice: (voiceId, voiceName) =>
        set({
      ...
      appendVoiceTranscript: (transcript) => {
        ...
        const newTranscript = voiceTranscript ? voiceTranscript + ' ' + transcript : transcript;
      ...
      setAutoSendSettings: (enabled, delayMs) =>
        set({
      ...
      setDeepgramApiKey: (key) => {
        ...
        fetch(`${getApiBaseUrl()}/api/config`, {
          ...
          console.error('[TaskStore] Failed to sync Deepgram key to backend:', err),
        );
      },
    ...
    }),
    {
      ...
      merge: (persistedState, currentState) => {
        ...
        return {
          ...
          expandedWorkspacesInitialized:
            persisted.expandedWorkspacesInitialized ?? persisted.expandedWorkspaces !== undefined,
          ...
          thinkingSoundInterval:
            persisted.thinkingSoundInterval ?? currentState.thinkingSoundInterval,
          voiceSummaryOnCompletion:
            persisted.voiceSummaryOnCompletion ?? currentState.voiceSummaryOnCompletion,
          voiceProgressUpdatesEnabled:
            persisted.voiceProgressUpdatesEnabled ?? currentState.voiceProgressUpdatesEnabled,
          voiceProgressUpdateInterval:
            persisted.voiceProgressUpdateInterval ?? currentState.voiceProgressUpdateInterval,
