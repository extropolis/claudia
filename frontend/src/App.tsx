...
function App() {
  ...
  const activeShellWorkspace = activeShellWorkspaceId
    ? workspaces.find((w) => w.id === activeShellWorkspaceId)
    : undefined;

  // Count tasks that have running processes (not disconnected or archived)
  const activeTasks = Array.from(tasks.values()).filter(
    (t) => t.state !== 'disconnected' && t.state !== 'archived' && t.state !== 'interrupted',
  );
  ...
  return (
    <div className={`app ${isMobile ? 'is-mobile' : ''}`}>
      <header className="app-header">
        ...
        <div className="header-controls">
          ...
          >
            ...
            {unreadTaskIds.size > 0 && <span className="activity-badge">{unreadTaskIds.size}</span>}
          </button>
          ...
          >
            {soundMuted ? (
              <BellOff size={isMobile ? 18 : 20} />
            ) : (
              <Bell size={isMobile ? 18 : 20} />
            )}
          </button>
          <button className="restart-button" onClick={handleRestartServer} title="Restart Server">
          ...
          <button className="settings-button" onClick={handleSettingsOpen} title="Settings">
          ...
          <>
            ...
            <section className="main-panel">
              ...
              {activeShellWorkspaceId && activeShellWorkspace && (
                <div
                  className="shell-terminal-wrapper"
                  style={{ display: showingShell ? 'flex' : 'none' }}
                >
                  <ShellTerminalView
                  ...
                  <>
                    {activeShellWorkspaceId && activeShellWorkspace && (
                      ...
                      >
                        ...
                        <span>
                          Shell running —{' '}
                          {activeShellWorkspace.displayName || activeShellWorkspace.name}
                        </span>
                      </button>
                    )}
                    <TerminalView
      ...
      <ProjectPicker
        ...
        wsRef={wsRef}
        requestRecentWorkspaces={requestRecentWorkspaces}
        clearRecentWorkspace={clearRecentWorkspace}
      />
      <SettingsMenu
        isOpen={showSettings}
      ...
      {!isOffline && (isServerReloading || !isConnected) && (
        <div className="server-reload-banner">
          ...
          <span>
            {isServerReloading ? 'Backend is restarting...' : 'Reconnecting to backend...'}
          </span>
        </div>
      )}
