...
export function getUnsupportedFeatureMessage(feature: string): string {
  const messages: Record<string, string> = {
    'directory-picker':
      ...
      '(Chrome 86+, Edge 86+).',
    clipboard: 'Clipboard access is not available. Please copy the text manually.',
  };
  return (
    messages[feature] || `This feature (${feature}) is not supported in your current environment.`
  );
}
    ...
    notification.onclick = () => {
      ...
      if (options?.data?.taskId) {
        window.dispatchEvent(
          new CustomEvent('notification:taskClick', {
...
export function sendTaskWaitingInputNotification(options: {
  ...
  const body = recentOutput
    ? recentOutput.length > 150
      ? recentOutput.substring(recentOutput.length - 150)
      : recentOutput
    : '';
