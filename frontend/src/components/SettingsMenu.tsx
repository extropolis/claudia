...
function CollapsiblePanel({ title, icon, isExpanded, onToggle, children }: CollapsiblePanelProps) {
  return (
    <div className="collapsible-panel">
      ...
      {isExpanded && <div className="collapsible-panel-content">{children}</div>}
    </div>
  );
}
...
export function SettingsMenu({ isOpen, onClose, initialPanel }: SettingsMenuProps) {
  ...
  const [notificationTestStatus, setNotificationTestStatus] = useState<'idle' | 'sent' | 'failed'>(
    'idle',
  );
  ...
  const [newServer, setNewServer] = useState({
    name: '',
    type: 'stdio' as 'stdio' | 'http' | 'streamableHttp',
    command: '',
    args: '',
    url: '',
    headers: '',
  ...
  const [customApiKeyTestStatus, setCustomApiKeyTestStatus] = useState<
    'idle' | 'testing' | 'success' | 'error'
  >('idle');
  const [customApiKeyTestMessage, setCustomApiKeyTestMessage] = useState('');

  // SAP AI Core test state
  const [sapAiCoreTestStatus, setSapAiCoreTestStatus] = useState<
    'idle' | 'testing' | 'success' | 'error'
  >('idle');
  ...
  const [hyperspaceTestStatus, setHyperspaceTestStatus] = useState<
    'idle' | 'testing' | 'success' | 'error'
  >('idle');
  ...
  useEffect(() => {
    const restoreSapAiCoreCredentials = async () => {
      ...
      const hasCredentials =
        sapAiCoreClientId && sapAiCoreClientSecret && sapAiCoreAuthUrl && sapAiCoreBaseUrl;
      ...
      if (hasCredentials) {
        try {
          ...
          if (checkResponse.ok) {
            ...
            if (needsRestore) {
              console.log(
                '[SettingsMenu] Restoring SAP AI Core credentials from localStorage to backend',
              );
      ...
      if (response.ok) {
        ...
        if (config.claudeCodeSwitches) {
          setCliSwitches({
            ...
            defaultModel:
              config.claudeCodeSwitches.defaultModel ||
              (config.claudeCodeSwitches as any).model ||
              '',
          });
        }
  ...
  const arrayToObjectFormat = useCallback(
    (
      servers: Array<{
        name: string;
        type?: string;
  ...
  const handleCliSwitchChange = useCallback(
    (updates: Partial<typeof cliSwitches>) => {
      setCliSwitches((prev) => {
  ...
  const handleCliSwitchToggle = useCallback(
    (updates: Partial<typeof cliSwitches>) => {
      setCliSwitches((prev) => {
        const updated = { ...prev, ...updates };
  ...
  const handleModelTieringToggle = useCallback(
    (enabled: boolean) => {
  ...
  const handleModelTieringTierChange = useCallback(
    (tier: 'low' | 'medium' | 'high', value: string) => {
      setModelTiering((prev) => {
  ...
  const saveApiMode = useCallback(
    async (mode: ApiMode, apiKey: string) => {
      try {
        const response = await fetch(`${getApiBaseUrl()}/api/config`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (!response.ok) {
          ...
          showWarning(
            ...
            'This change only applies to new tasks. Running tasks will continue using their original configuration.',
          );
        }
      } catch (error) {
      ...
      if (response.ok) {
        ...
        if (pluginsResponse.ok) {
          ...
          if (pluginsData.success && pluginsData.plugins) {
            setPlugins(pluginsData.plugins);
            console.log(
              `[Plugin Toggle] Plugin list refreshed, total plugins:`,
    ...
    <div
      ...
      onMouseDown={(e) => {
        if (e.target === e.currentTarget)
          (e.currentTarget as HTMLElement).dataset.closeOnMouseup = '1';
      }}
      onMouseUp={(e) => {
        if (
          (e.currentTarget as HTMLElement).dataset.closeOnMouseup === '1' &&
          ...
          onClose();
        delete (e.currentTarget as HTMLElement).dataset.closeOnMouseup;
      }}
    >
        ...
        <div className="settings-menu-content">
          ...
          >
            <div style={{ padding: '8px 0' }}>
              <UsageDashboard
                isOpen={expandedPanels.usage}
                onClose={() => togglePanel('usage')}
                embedded
              />
            </div>
          </CollapsiblePanel>
              ...
              <div className="permission-item">
                ...
                <label className="toggle-switch">
                  <input
                    ...
                    disabled={
                      !hasBrowserNotifications() || getNotificationPermission() === 'denied'
                    }
                    onChange={async (e) => {
                      if (e.target.checked) {
                ...
                >
                  ...
                  {notificationTestStatus === 'sent' && (
                    <p
                      ...
                      style={{ marginTop: 8, fontSize: '0.75rem', color: 'var(--text-secondary)' }}
                    >
            ...
            <div className="permissions-content">
              ...
              <div className="permission-item">
                <div className="permission-info">
                  ...
                  <span className="permission-description">
                    Default directory for new workspaces. Enter just a folder name to create
                    workspaces under this directory.
                  </span>
                </div>
                <input
                  type="text"
          ...
          >
            <div className="api-config-content">
              <p className="api-config-description">Choose which AI coding assistant to use.</p>
              ...
              <div className="api-mode-selector">
                ...
                <label className={`api-mode-option ${backend === 'opencode' ? 'selected' : ''}`}>
                  ...
                  <div className="api-mode-content">
                    ...
                    <span className="api-mode-description">Open-source AI coding agent by SST</span>
                  </div>
                </label>
              </div>
              ...
              {backendStatusLoading ? (
                ...
                backendStatus && (
                  <div
                    className={`backend-status ${backendStatus.installed ? 'installed' : 'not-installed'}`}
                  >
              ...
              <p className="api-config-note">
                Note: Changing backends requires restarting tasks. Existing tasks will continue with
          ...
          >
            <div className="api-config-content">
              ...
              <div className="api-mode-selector">
                ...
                <label
                  className={`api-mode-option ${apiMode === 'custom-anthropic' ? 'selected' : ''}`}
                >
                  <input
                    type="radio"
                    name="apiMode"
                ...
                {plugins
                  .filter((p) => p.type === 'ai-provider' && p.enabled)
                  .map((plugin) => (
                    <label
                      key={plugin.name}
                      className={`api-mode-option ${apiMode === plugin.apiMode ? 'selected' : ''}`}
                    >
                      <input
                        type="radio"
                        name="apiMode"
                      ...
                      <div className="api-mode-content">
                        ...
                        <span className="api-mode-description">
                          {plugin.name === 'sap-ai-core-plugin' &&
                            'Use SAP AI Core with your own credentials'}
                          {plugin.name === 'hai-proxy-plugin' &&
                            'Use HAI Proxy for Anthropic Claude models'}
                        </span>
                      </div>
                    </label>
                  ))}
                ...
                <div className="api-mode-fields">
                  ...
                  {customApiKeyTestStatus !== 'idle' && (
                    <div className={`aicore-test-result ${customApiKeyTestStatus}`}>
                      {customApiKeyTestStatus === 'testing' && (
                        <Loader2 size={16} className="spinning" />
                      )}
              ...
              {apiMode === 'sap-ai-core' && (
                <div className="api-mode-fields">
                  ...
                  <div className="aicore-buttons">
                    ...
                    <button className="aicore-test-btn" onClick={refreshSapAiCoreModels}>
                      Refresh Models
                    </button>
                    <button className="aicore-test-btn" onClick={saveSapAiCoreConfig}>
                      Save Configuration
                    </button>
                  </div>
                </div>
              ...
              {apiMode === 'hyperspace-proxy' && (
                <div className="api-mode-fields">
                  ...
                  {/* Proxy Status */}
                  <div
                    className={`aicore-test-result ${hyperspaceProxyStatus.proxyRunning ? 'success' : 'idle'}`}
                  >
                    <span>
                      Proxy Status:{' '}
                      {hyperspaceProxyStatus.loading
                        ? 'Checking...'
                        : hyperspaceProxyStatus.proxyRunning
                          ? '🟢 Running'
                          : hyperspaceProxyStatus.haiInstalled
                            ? '🔴 Stopped'
                            : '⚠️ HAI CLI Not Installed'}
                    </span>
                  </div>
                  ...
                  <div className="aicore-buttons">
                    ...
                    <button className="aicore-test-btn" onClick={refreshHyperspaceModels}>
                      Refresh Models
                    </button>
                    <button
                      ...
                      onClick={
                        hyperspaceProxyStatus.proxyRunning
                          ? stopHyperspaceProxy
                          : startHyperspaceProxy
                      }
                      disabled={
                        hyperspaceProxyStatus.loading || !hyperspaceProxyStatus.haiInstalled
                      }
                    >
                      {hyperspaceProxyStatus.loading
                        ? 'Loading...'
                    ...
                    <button className="aicore-test-btn" onClick={saveHyperspaceProxyConfig}>
                      Save Configuration
                    </button>
                  </div>
                </div>
              ...
              <p className="api-config-note">
                Note: The server must be restarted after changing API mode for the changes to take
                effect on new tasks.
              </p>
            </div>
          </CollapsiblePanel>
                          ...
                          <div key={server.name} className="mcp-server-item">
                            <div className="mcp-server-info">
                              <div className="mcp-server-name-row">
                                ...
                                <span
                                  className={`mcp-server-type ${server.type === 'streamableHttp' ? 'http' : 'stdio'}`}
                                >
                  ...
                  {isAddingServer ? (
                    <div className="mcp-add-form">
                      ...
                      <div className="mcp-type-selector">
                        <label
                          className={`mcp-type-option ${newServer.type === 'stdio' ? 'selected' : ''}`}
                        >
                          <input
                            type="radio"
                            name="serverType"
                        ...
                        <label
                          className={`mcp-type-option ${newServer.type === 'streamableHttp' ? 'selected' : ''}`}
                        >
                          <input
                            type="radio"
                            name="serverType"
                            ...
                            onChange={() =>
                              setNewServer((prev) => ({ ...prev, type: 'streamableHttp' }))
                            }
                          />
                          <span>HTTP (URL)</span>
                        </label>
                      ...
                      {newServer.type === 'stdio' ? (
                        <>
                          <input
                            ...
                            onChange={(e) =>
                              setNewServer((prev) => ({ ...prev, command: e.target.value }))
                            }
                            className="mcp-input"
                          />
                          <input
                            ...
                            onChange={(e) =>
                              setNewServer((prev) => ({ ...prev, args: e.target.value }))
                            }
                            className="mcp-input"
                          />
                        </>
                      ...
                      <div className="mcp-add-form-actions">
                        ...
                        <button
                          ...
                          disabled={
                            !newServer.name ||
                            (newServer.type === 'stdio' ? !newServer.command : !newServer.url)
                          }
                        >
                          Add Server
                        </button>
                    ...
                    <button className="mcp-add-btn" onClick={() => setIsAddingServer(true)}>
                      <Plus size={16} />
                      Add MCP Server
                    </button>
                  )}
          ...
          >
            <div className="plugins-content">
              <div className="plugins-description">
                Enable or disable plugins to extend Claudia's functionality. Plugins are disabled by
            ...
            <div className="permissions-content">
              <div className="permission-item">
                <div className="permission-info">
                  ...
                  <span className="permission-description">
                    Automatically approve all Claude actions without prompts. Use with caution -
              ...
              {skipPermissions && (
                <div className="permission-warning">
                  Warning: Claude can execute any command without confirmation. Only enable in
            ...
            <div className="permissions-content cli-switches-content">
              ...
              <div className="permission-item">
                <div className="permission-info">
                  ...
                  <span className="permission-description">
                    Controls how much reasoning Claude invests before responding. Higher = more
                    thinking tokens.
                  </span>
                </div>
                <select
              ...
              <div className="permission-item">
                <div className="permission-info">
                  ...
                  <span className="permission-description">
                    Model used for new tasks. Passed as --model to Claude Code CLI. Use the text box
                    for custom model IDs.
                  </span>
                </div>
                <select
                  className="cli-switch-select"
                  value={
                    [
                      ...
                      'sonnet',
                      'haiku',
                    ].includes(cliSwitches.defaultModel || '')
                      ? cliSwitches.defaultModel || ''
                      : '__custom__'
                  }
                  onChange={(e) => {
                    if (e.target.value !== '__custom__')
                      handleCliSwitchToggle({ defaultModel: e.target.value });
                  }}
                >
              ...
              <div className="permission-item">
                <div className="permission-info">
                  ...
                  <span className="permission-description">
                    Let agents passing through the Claudia MCP tag spawned tasks with a complexity
                    tier (low/medium/high) that maps to a cheaper or stronger model. Use values
                    valid for your current API mode — for SAP AI Core or Hyperspace proxies, use
                    full model IDs (e.g. claude-3-5-haiku-20251001).
                  </span>
                </div>
                <label className="toggle-switch">
                  <input
                ...
                <div className="cli-switch-input-group">
                  <label className="toggle-switch">
                    <input
                      ...
                      onChange={(e) =>
                        handleCliSwitchToggle({ maxTurns: e.target.checked ? 50 : null })
                      }
                    />
                    <span className="toggle-slider"></span>
                  </label>
                  {cliSwitches.maxTurns !== null && (
                    <input
                      ...
                      max={1000}
                      onChange={(e) =>
                        handleCliSwitchChange({ maxTurns: parseInt(e.target.value) || 1 })
                      }
                    />
                  )}
                </div>
                ...
                <div className="cli-switch-input-group">
                  <label className="toggle-switch">
                    <input
                      ...
                      onChange={(e) =>
                        handleCliSwitchToggle({ maxBudgetUsd: e.target.checked ? 5.0 : null })
                      }
                    />
                    <span className="toggle-slider"></span>
                  </label>
                  {cliSwitches.maxBudgetUsd !== null && (
                    <input
                      ...
                      step={0.5}
                      onChange={(e) =>
                        handleCliSwitchChange({ maxBudgetUsd: parseFloat(e.target.value) || 0 })
                      }
                    />
                  )}
                </div>
              ...
              <div className="permission-item">
                <div className="permission-info">
                  ...
                  <span className="permission-description">
                    Set permission mode: plan (read-only), acceptEdits (auto-approve edits), dontAsk
                    (auto-approve all).
                  </span>
                </div>
                <div className="cli-switch-input-group">
                  <label className="toggle-switch">
                    <input
                      ...
                      onChange={(e) =>
                        handleCliSwitchToggle({
                          permissionMode: e.target.checked ? 'dontAsk' : null,
                        })
                      }
                    />
                    <span className="toggle-slider"></span>
            ...
            <div className="permissions-content">
              <div className="permission-item">
                <div className="permission-info">
                  ...
                  <span className="permission-description">
                    When enabled, relevant learnings from past conversations will be automatically
                    retrieved and injected into new task contexts using semantic search (RAG).
                  </span>
                </div>
                <label className="toggle-switch">
                  <input
              ...
              <p className="api-config-note" style={{ marginTop: '12px' }}>
                To add learnings, click the "Learn" button on a completed task. Learnings are stored
                with embeddings and matched based on semantic similarity to new task prompts.
              </p>
            </div>
          </CollapsiblePanel>
            ...
            <div className="permissions-content">
              <div className="permission-item">
                <div className="permission-info">
                  ...
                  <span className="permission-description">
                    When enabled, Claude Code sessions spawned by Claudia will have access to MCP
                    tools for interacting with the orchestrator &mdash; creating new tasks, listing
                    workspaces, checking task status, and delegating work to parallel agents.
                  </span>
                </div>
                <label className="toggle-switch">
                  <input
              ...
              <p className="api-config-note" style={{ marginTop: '12px' }}>
                ...
                <code>claudia_create_task</code>,<code>claudia_send_input</code>,{' '}
                <code>claudia_continue_task</code>,<code>claudia_stop_task</code>,{' '}
                <code>claudia_rename_task</code>. Tasks are scoped to the current workspace.
