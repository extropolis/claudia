...
export function FileContentModal({
  ...
  const formatDiffLine = (line: string, index: number) => {
    if (line.startsWith('+++') || line.startsWith('---')) {
      return (
        <div key={index} className="diff-line diff-header">
          {line}
        </div>
      );
      ...
      return (
        <div key={index} className="diff-line diff-remove">
          {line}
        </div>
      );
    }
      ...
      <div className="file-content-modal">
        ...
        <div className="file-content-modal-body">
          ...
          {!loading &&
            ...
            content &&
            !isImage &&
            !(isMarkdownFile && showRendered && !isDiff) && (
          ...
          {!loading && !error && !isEditing && !content && (
            <div className="file-content-empty">{isDiff ? 'No changes' : 'Empty file'}</div>
          )}
        </div>
      </div>
    </div>,
