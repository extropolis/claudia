...
export function ProjectPicker({
    ...
    const handler = (event: MessageEvent) => {
      try {
        ...
        if (message.type === 'workspace:recent:list') {
          console.log(
            '[ProjectPicker] Received recent workspaces:',
            message.payload.recentWorkspaces,
          );
  ...
  const handleFolderSelect = useCallback(async () => {
    try {
      ...
      if (isLocalhost) {
        console.log(
          '[ProjectPicker] Localhost detected, showing path input modal with backend browse option',
        );
