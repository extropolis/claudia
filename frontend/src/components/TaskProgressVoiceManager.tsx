...
export function TaskProgressVoiceManager() {
                ...
                if (!progressTimersRef.current.has(task.id)) {
                    ...
                    const timer = setInterval(() => {
                        ...
                        if (!currentTask || currentTask.state !== 'busy') {
                            ...
                            console.log('[TaskProgressVoiceManager] Task no longer busy, stopping tracker:', task.id);
                        ...
                        fetch(`${getApiBaseUrl()}/api/tasks/${task.id}/conversation`)
                            ...
                            .then(data => {
                                ...
                                if (lastAssistant?.content) {
                                    ...
                                    const firstSentence = snippet.match(/^[^.!?]+[.!?]/)?.[0] || snippet.substring(0, 80);
    ...
    }, [tasks, voiceProgressUpdatesEnabled, voiceProgressUpdateInterval, globalVoiceEnabled, speak, cancel]);
