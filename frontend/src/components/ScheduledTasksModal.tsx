...
export function ScheduledTasksModal({
  ...
  const [formMode, setFormMode] = useState<'hidden' | 'create' | 'edit'>(
    hasInitialPrompt ? 'create' : 'hidden',
  );
    ...
    <div className="modal-overlay scheduled-tasks-overlay" onClick={onClose}>
      <div className="modal-content scheduled-tasks-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="scheduled-tasks-description">
          Manage scheduled prompts for <strong>{taskName}</strong>. Prompts are sent to the task at
          scheduled times when it is idle.
        </p>
        ...
        <div className="scheduled-tasks-list">
          ...
          {scheduledTasks.map((st: ScheduledTask) => (
            <div
              key={st.id}
              className={`scheduled-task-item ${editingId === st.id ? 'editing' : ''} ${st.isPaused ? 'paused' : ''}`}
            >
              <div className="scheduled-task-info">
                <div className="scheduled-task-top">
                  {st.isPaused && <span className="scheduled-task-badge paused">paused</span>}
                  <span
                    className={`scheduled-task-badge ${st.isRecurring ? 'recurring' : 'one-shot'}`}
                  >
                ...
                <div className="scheduled-task-meta">
                  <span title={st.nextFireAt || ''}>
                    {st.isPaused ? 'Paused' : `Next: ${formatNextFire(st.nextFireAt)}`}
                  </span>
        ...
        {formMode !== 'hidden' ? (
          ...
          <div className="scheduled-tasks-footer">
            ...
            <button
              className="btn-secondary scheduled-tasks-refresh"
