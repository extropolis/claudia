...
/** The draggable ancestor of a rendered label (workspace header / task row). */
function draggableFor(label: string): HTMLElement {
    const el = screen.getByText(label).closest('[draggable="true"]');
    if (!el) throw new Error(`no draggable ancestor for "${label}"`);
