...
describe('SettingsMenu — config round-trip', () => {
    ...
    it('populates the Behavior panel from the fetched config', async () => {
        ...
        expect(controlFor('Archived Worktree Retention (days)')).toHaveValue(14);
    });
    ...
    it('falls back to defaults for a sparse config payload', async () => {
        ...
        // worktreeRetentionDays defaults to 30 when the backend omits it.
        expect(controlFor('Archived Worktree Retention (days)')).toHaveValue(30);
    });
    ...
    it('clamps a negative worktree retention to 0 before persisting', async () => {
        installFetch(fullConfig);
        await renderSettings({ initialPanel: 'behavior' });

        const input = controlFor('Archived Worktree Retention (days)');
        fireEvent.change(input, { target: { value: '-5' } });
        fireEvent.blur(input);
        await flush();

        expect(putBodies()).toContainEqual({ worktreeRetentionDays: 0 });
        expect(input).toHaveValue(0);
    });

    it('treats a cleared retention field as 0 rather than NaN', async () => {
        installFetch(fullConfig);
        await renderSettings({ initialPanel: 'behavior' });
