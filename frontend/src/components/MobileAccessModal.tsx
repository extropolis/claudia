...
export function MobileAccessModal({
  ...
  const mobileUrl =
    status?.active && status?.url && status?.token ? `${status.url}/?token=${status.token}` : '';

  return (
    <div className="mobile-access-overlay" onClick={onClose}>
      <div
        className="mobile-access-modal"
      ...
      >
        ...
        <div className="tunnel-status">
          <span
            className={`status-dot ${status?.active ? 'active' : error ? 'error' : tunnelLoading ? 'starting' : 'inactive'}`}
          />
          <span>
            {status?.active
              ? 'Tunnel active'
        ...
        <div className="qr-code-area">
          {status?.active ? (
            ...
            <div className="qr-placeholder error">Failed</div>
          ) : tunnelLoading ? (
            <div className="qr-placeholder">Starting...</div>
          ) : (
            <div className="qr-placeholder">Not connected</div>
          )}
        </div>
