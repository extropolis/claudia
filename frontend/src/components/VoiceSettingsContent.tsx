...
export function VoiceSettingsContent() {
  ...
  const [localThinkingSoundInterval, setLocalThinkingSoundInterval] = useState(
    thinkingSoundInterval / 1000,
  );
  const [localProgressUpdateInterval, setLocalProgressUpdateInterval] = useState(
    voiceProgressUpdateInterval / 1000,
  );
  ...
  const saveVoiceSettings = useCallback(
    (voice: string, rate: number, pitch: number, volume: number) => {
      setVoiceSettings({
  ...
  return (
    <div className="voice-settings-content">
      ...
      {autoSendEnabled && (
        <div className="settings-section">
          <label className="setting-label">Auto-Send Delay: {localAutoSendDelay.toFixed(1)}s</label>
          <input
            type="range"
            min="0.5"
            max="5"
          ...
          <p className="setting-description" style={{ marginTop: '8px', fontSize: '12px' }}>
            Message will be sent automatically after {localAutoSendDelay.toFixed(1)} seconds of
            silence
          </p>
        </div>
      )}
      ...
      {voiceProgressUpdatesEnabled && (
        <div className="settings-section">
          <label className="setting-label">
            Update Interval: {Math.floor(localProgressUpdateInterval / 60)}m{' '}
            {Math.floor(localProgressUpdateInterval % 60)}s
          </label>
          <input
            type="range"
            min="60"
          ...
          <p className="setting-description" style={{ marginTop: '8px', fontSize: '12px' }}>
            Progress will be announced every {Math.floor(localProgressUpdateInterval / 60)} minutes{' '}
            {Math.floor(localProgressUpdateInterval % 60) > 0
              ? `and ${Math.floor(localProgressUpdateInterval % 60)} seconds`
              : ''}{' '}
            for busy tasks
          </p>
        </div>
      ...
      <div className="settings-section">
        ...
        <p className="setting-description">
          Play a gentle notification sound at regular intervals while any task is actively thinking
          or processing
        </p>
      </div>
      ...
      {thinkingSoundEnabled && (
        <div className="settings-section">
          ...
          <p className="setting-description" style={{ marginTop: '8px', fontSize: '12px' }}>
            Sound will play every {localThinkingSoundInterval.toFixed(1)} seconds while Claude is
            thinking
          </p>
        </div>
      )}
      ...
      <div className="settings-section">
        <label className="setting-label">Speed: {localRate.toFixed(1)}x</label>
        <input
          type="range"
          min="0.5"
          max="2"
      ...
      <div className="settings-section">
        <label className="setting-label">Pitch: {localPitch.toFixed(1)}</label>
        <input
          type="range"
          min="0.5"
          max="2"
      ...
      <div className="settings-section">
        <label className="setting-label">Volume: {Math.round(localVolume * 100)}%</label>
        <input
          type="range"
          min="0"
          max="1"
