...
const taskRow = (page: import('@playwright/test').Page) =>
    page.locator('[data-testid="task-item"]').filter({ hasText: PROMPT });
...
test('history survives repeated reloads without duplicating', async ({ page }) => {
    await openApp(page);
    await selectTask(page, taskRow(page));
    await expectTerminalToContain(page, BEFORE_MARKER);
