...
import {
    ...
    fakeStdin, openApp, selectTask, sendFollowUp, taskRow,
} from '../harness/ui.js';
...
test('creates a task and spawns the CLI with the prompt', async ({ page }) => {
    ...
    // The row renders the backend's fallback display title (derived from the
    // prompt, e.g. "E2ETASKPROMPTALPHA") plus a `.task-ref` badge, so the
    // visible text is not the prompt and its exact shape is not this suite's
    // contract. `title` carries the prompt verbatim — assert that exactly, and
    // only that the row shows *something*.
    const promptEl = item.getByTestId('task-prompt');
    await expect(promptEl).toHaveAttribute('title', PROMPT);
    await expect(promptEl).not.toBeEmpty();
...
test('archives the task and it leaves the active list', async ({ page }) => {
    ...
    await expect(taskRow(page, PROMPT)).toHaveCount(0);
});
