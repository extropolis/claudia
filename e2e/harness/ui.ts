...
/**
 * The sidebar row for the task created with `prompt`.
 *
 * Deliberately NOT `filter({ hasText: prompt })`: the backend assigns a
 * fallback display title derived from the prompt (underscores stripped, etc.),
 * and the row renders that title plus a `.task-ref` badge — so the visible text
 * is neither stable nor equal to the prompt. `task-prompt`'s `title` attribute
 * carries the untransformed prompt verbatim, which is the one exact hook.
 */
export function taskRow(page: Page, prompt: string): Locator {
    return page
        .locator('[data-testid="task-item"]')
        .filter({ has: page.locator(`[data-testid="task-prompt"][title="${cssEscape(prompt)}"]`) });
}
