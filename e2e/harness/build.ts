...
export function buildStack(): void {
    ...
    //
    // `vite build` directly, not `npm run build -w frontend` (= `tsc && vite
    // build`): typechecking is already gated by tests.yml, and coupling the E2E
    // run to it means an unrelated type error anywhere in the frontend takes the
    // whole browser suite offline instead of failing the check that owns it.
    //
